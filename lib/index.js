...
async function render (context, appState, options) {
    ...
    const requestUrl = encodeURI(context.req.url);
    ...
    if (jwtCookie) {
        fetchOptionsToPass.headers = {
            'Cookie': `oauth_jwt=${jwtCookie}`
        };
    }
    ...
    const store = createStore(combineReducers({...options.reducers}), appState, applyMiddleware(thunk));
            ...
            result = renderToString(
                React.createElement(Provider, { store },
                    React.createElement(StaticRouter, { location: requestUrl, context }, 
                        React.createElement(App, { renderType: 'shallow', registerFetchAction: registerFetchAction(actions) }, null)
                    )
                )
            );
        } catch (err) {
