...
const cacheTime = 60 * 60 * 24 * 7; // In seconds
...
// options.App = App
// options.preloadAll = [bool]; // 
// options.useShallowRender = [bool]; // 
...
async function render (context, appState, options) {
    ...
    // This will make sure react loadable handles ssr for loadable components
    if (options.preloadAll) {
        await Loadable.preloadAll();
    }

    // If we want to use the shallow component helper (that doesnt render everything completely), we ca do it here
    let shallowRender = options.useShallowRender === true ? 'shallow' : '';
            ...
            result = renderToString(
                React.createElement(Provider, { store },
                    React.createElement(StaticRouter, { location: requestUrl, context }, 
                        React.createElement(options.App, { renderType: shallowRender, registerFetchAction: registerFetchAction(actions) }, null)
                    )
                )
            );
        } catch (err) {
