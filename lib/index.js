...
const RegisterFetchActionProvider = require('../register-fetch-action-provider/server');
    ...
    while (actions.length > previousActionLength) {
        ...
        let allFetch = actions.reduce((promise, action) => {
            ...
            return promise.then(() => {
                ...
                return action.fn(actionArguments)
                    .then(result => {
                        action.resolved = true;
                        if (result &&  result.noIndex) {
                            context.set('X-Robots-Tag', 'noindex, nofollow');
                        }
