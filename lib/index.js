...
async function render (context, appState, options) {
    ...
    const jwtCookie = context.cookies.get('oauth_jwt');

    if (jwtCookie) {
        fetchOptionsToPass.headers = fetchOptionsToPass.headers || {};
        fetchOptionsToPass.headers.Cookie = fetchOptionsToPass.headers.Cookie || '';
        fetchOptionsToPass.headers.Cookie += `oauth_jwt=${jwtCookie};`;
    }

    // This is not super good right now, but we will refactor it later:
    // Check the same as above, but with another cookie name
    const jwtCookie2 = context.cookies.get('session_jwt');
    if (jwtCookie) {
        fetchOptionsToPass.headers = fetchOptionsToPass.headers || {};
        fetchOptionsToPass.headers.Cookie = fetchOptionsToPass.headers.Cookie || '';
        fetchOptionsToPass.headers.Cookie += `oauth_jwt=session_jwt${jwtCookie2};`;
    }
    ...
    fetchOptionsToPass.headers['x-site-id'] = context.request.header['x-site-id'];
    ...
    const key = `${context.language}-${device}-${requestUrl}-${browser}`;
